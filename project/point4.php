	...
	$uniqueCapacity = "";
        ...
        if (isset($_POST["insert"])) {
            ...
            $year = $_POST["selectyear"];
                ...
                //ALL Room Capacity   
				$allCapacity = $con->query("SELECT ROOM_CAPACITY FROM class_size WHERE  Semester='$semester' AND Year='$year'");
				...
				while($row = mysqli_fetch_array($allCapacity)){
					$totalEnrolled=$row['ROOM_CAPACITY'];
				...
				foreach($uniqueCapacity as $cap){
                    ...
                    $sql = "SELECT SUM(ENROLLED) FROM class_size WHERE ROOM_CAPACITY='$cap' AND Semester='$semester' AND Year='$year'";
                    $result = $con->query($sql);
                    $count = mysqli_num_rows($result);
                    $totalEnrolled=0;
                    while($row = mysqli_fetch_array($result)){
                         $totalEnrolled=$cap*$capacityTimes;
