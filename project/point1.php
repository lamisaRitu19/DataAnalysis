   ...
   //COFFERED_WITH CAPACITY
	...
	$uniqueCapacity = "";
        ...
        if (isset($_POST["insert"])) {
			...
			//ALL Room Capacity   
			$allCapacity = "SELECT ROOM_CAPACITY FROM class_size WHERE  Semester='$semester' AND Year='$year'";
			...
			foreach($result2 as $row){
				array_push($capacityArray,$row['ROOM_CAPACITY']);
			}
			...
			foreach($uniqueCapacity as $cap){
				...
				$allSections = "SELECT COUNT(COFFERED_WITH) FROM class_size WHERE ROOM_CAPACITY='$cap' AND Semester='$semester' AND Year='$year'";
				...
				$inOneYear = number_format((float)($totalSections/12), 2, '.', '');
				
				array_push($countOfSections, array($cap,$totalSections,$inOneYear));
